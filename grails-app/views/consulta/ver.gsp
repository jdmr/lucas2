	...
	<body>
		...
		<div id="show-consulta" class="content scaffold-show" role="main">
			...
			<ol class="property-list consulta">
				...
				<g:if test="${consulta?.dateCreated}">
				<li class="fieldcontain">
					<span id="dateCreated-label" class="property-label"><g:message code="consulta.dateCreated.label" default="Date Created" /></span>
					
						<span class="property-value" aria-labelledby="dateCreated-label"><g:formatDate date="${consulta?.dateCreated}" /></span>
					
				</li>
				</g:if>
			
				<g:if test="${consulta?.doctor}">
				<li class="fieldcontain">
					<span id="doctor-label" class="property-label"><g:message code="consulta.doctor.label" default="Doctor" /></span>
					
						<span class="property-value" aria-labelledby="doctor-label"><g:link controller="doctor" action="show" id="${consulta?.doctor?.id}">${consulta?.doctor?.encodeAsHTML()}</g:link></span>
					
				</li>
				</g:if>
			
				<g:if test="${consulta?.examenFisicos}">
				<li class="fieldcontain">
					<span id="examenFisicos-label" class="property-label"><g:message code="consulta.examenFisicos.label" default="Examen Fisicos" /></span>
					
						<g:each in="${consulta.examenFisicos}" var="e">
				...
				<g:if test="${consulta?.lastUpdated}">
				<li class="fieldcontain">
					<span id="lastUpdated-label" class="property-label"><g:message code="consulta.lastUpdated.label" default="Last Updated" /></span>
					
						<span class="property-value" aria-labelledby="lastUpdated-label"><g:formatDate date="${consulta?.lastUpdated}" /></span>
					
				</li>
				</g:if>
			
				<g:if test="${consulta?.paciente}">
				<li class="fieldcontain">
					<span id="paciente-label" class="property-label"><g:message code="consulta.paciente.label" default="Paciente" /></span>
					
						<span class="property-value" aria-labelledby="paciente-label"><g:link controller="paciente" action="show" id="${consulta?.paciente?.id}">${consulta?.paciente?.encodeAsHTML()}</g:link></span>
			...
			<g:form>
				<fieldset class="buttons">
					<g:hiddenField name="id" value="${consulta?.id}" />
					<g:link class="edit" action="edita" id="${consulta?.id}"><g:message code="default.button.edit.label" default="Edit" /></g:link>
